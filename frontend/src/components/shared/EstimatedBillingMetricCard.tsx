...
interface EstimatedBillingMetricCardProps {
...
export function EstimatedBillingMetricCard({
  data,
  loading,
  className,
  onMaximize,
    ...
    return (
      <Card className={cn('min-h-[320px]', className)}>
        <CardContent className="flex min-h-[320px] flex-col space-y-4 p-5">
          ...
          <Skeleton className="h-4 w-44 self-center" />
      ...
      <CardContent className="flex min-h-[320px] flex-col p-5">
        <div className="flex min-h-8 items-center justify-between gap-2">
          ...
          <Button
            ...
            aria-label="Ampliar facturación estimada"
          >
        ...
        <div className="mt-2 flex items-baseline justify-between gap-4">
          <span className="text-3xl font-bold tracking-tight text-[#0f172a] dark:text-gray-50">
            {formatCurrencyCompact(data?.currentTotal ?? 0)}
          </span>
          {changePct != null ? (
            <span
              className={cn(
