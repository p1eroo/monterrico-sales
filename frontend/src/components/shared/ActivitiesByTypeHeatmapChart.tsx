...
interface ActivitiesByTypeHeatmapChartProps {
...
export function ActivitiesByTypeHeatmapChart({
  data,
  scopeLabel = 'Equipo completo',
  className,
  chartHeight,
}: ActivitiesByTypeHeatmapChartProps) {
    ...
    () => ({
      ...
      tooltip: {
        ...
        custom: ({ seriesIndex, dataPointIndex }) => {
          const typeLabel = data.series[seriesIndex]?.name ?? 'Actividad';
          const week = data.weeks[dataPointIndex] ?? '';
          const value = Number(
            data.series[seriesIndex]?.data[dataPointIndex]?.y ?? 0,
          );
          const label = formatActivityCount(typeLabel, value);
