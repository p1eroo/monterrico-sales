...
export type CampaignBuilderShellProps = {
  source: CampaignAudienceSource;
  /** Base de navegación del módulo de campañas (historial). */
  basePath?: string;
  /** Permiso opcional para crear/enviar. Si no se pasa, está disponible para todos. */
...
export default function CampaignBuilderShell({
  ...
  const [audienceFilters, setAudienceFilters] = useState<CampaignAudienceFilters>(
    source.defaultFilters,
  );
  const [candidates, setCandidates] = useState<CampaignRecipient[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const c = await getCampaignApi(idToLoad, area);
        if (cancelled) return;
  ...
  }, [draftIdFromState, duplicateId, location.key, area]);

  const loadCandidates = useCallback(async () => {
    setCandidatesLoading(true);
    setCandidatesError(null);
    try {
      const rows = await source.loadCandidates();
      setCandidates(rows);
    } catch {
      setCandidatesError('No se pudieron cargar los candidatos.');
  ...
  const filteredCandidates = useMemo(() => {
    const q = (audienceFilters.search ?? '').trim().toLowerCase();
    const etapa = audienceFilters.etapa ?? '';
    const empresa = (audienceFilters.empresa ?? '').trim().toLowerCase();
    return candidates.filter((c) => {
      ...
      if (empresa && !(c.company ?? '').toLowerCase().includes(empresa)) return false;
      return true;
    });
  }, [candidates, audienceFilters]);
  ...
  const campaignEntityFromForm = useCallback(
    ...
    [campaignName, message, recipients, currentUser.id, currentUser.name, editingCampaignId, area],
  );
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (editingCampaignId) {
        await updateCampaignApi(editingCampaignId, payload, area);
  ...
  const addFromCandidates = useCallback(
    (ids: string[]) => {
      const already = (r: CampaignRecipient, c: CampaignRecipient) =>
        r.id === c.id ||
  ...
  const loadTemplate = (t: CampaignMessageTemplate) => {
    ...
    const body = t.editorJson ? t.body : plainTextToHtmlForEmail(t.body);
    setMessage({
      channel: 'email',
      subject: t.subject,
      body,
  ...
  const persistCampaignAfterSend = async (draft: Campaign, messageWithAttachments: CampaignMessage) => {
    ...
    try {
      ...
      if (previousDraftId) {
        try {
          await deleteCampaignApi(previousDraftId, area);
        } catch {
